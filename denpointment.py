...
app.config["MYSQL_HOST"] = "srv1669.hstgr.io"
app.config["MYSQL_USER"] = "u837608823_denpointment"
app.config["MYSQL_PASSWORD"] = "Gwyneth@0925"
app.config["MYSQL_DB"] = "u837608823_denpointment"
...
def list_users():
    ...
    cur.execute("SELECT ssn, first_name, last_name FROM persons WHERE ssn NOT IN (SELECT dentist_ssn FROM dentists)")
    users = cur.fetchall()
    # print(users)
    cur.close()
    cur = mysql.connection.cursor()
    cur.execute("SELECT p.ssn, p.first_name, p.last_name FROM persons p JOIN dentists d ON p.ssn = d.dentist_ssn;")
    dentists = cur.fetchall()
    cur.close()

    return render_template("admin/users.html", users=users, dentists=dentists)
...
@app.route("/admin/assign-room", methods=["POST"])
def assign_room():
    if not session.get("admin_logged_in"):
        return redirect(url_for("admin_login"))

    doctor_ssn = request.form['doctor_ssn']
    room_number = request.form['room_number']

    cur = mysql.connection.cursor()
    cur.execute("INSERT INTO dentists (dentist_ssn, room_number) VALUES (%s, %s)", (doctor_ssn, room_number))
...
def register():
    ...
    # JSON-based registration (like from Thunder Client)
    if request.is_json:
        ...
        query = "INSERT INTO persons(email, password, first_name, last_name, gender, birth_date) VALUES(%s, %s, %s, %s, %s, %s)"
        cursor.execute(query, (email, password, first_name, last_name, gender, birth_date))
        mysql.connection.commit()
        cursor.close()

        return jsonify({"status": "success", "message": "Registration successful"}), 201

    # Form-based registration (HTML form)
        ...
        if same_email:
            flash(message="This email has been registered, if you think there was a mistake then contact 'Tzesh'", category="danger")
            cursor.close()
            return redirect(url_for("index"))
...
def modify_phone(number):
    ...
    if request.method == "GET":
        ...
        if result == 0:
            flash(message="There's no such phone number stored in your profile.", category="danger")
            return redirect(url_for("profile"))
        else:
    ...
    if request.method == "POST" and form.validate():
        ...
        cursor = mysql.connection.cursor()
        query = "SELECT * FROM person_contact_numbers WHERE contact_number = %s"
        result = cursor.execute(query, (phone_number,))

        if result:
            cursor.close()
            flash(message="This phone number has already been registered.", category="danger")
            return render_template("modify-phone.html", form=form)
        else:
